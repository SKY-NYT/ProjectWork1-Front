...
const AttendanceLogs = () => {
  ...
  const handleNewAttendanceRecord = useCallback((data) => {
    ...
    const newRecord = {
      ...
      studentName:
        studentInfo?.name || attendanceRecord.studentName || "Unknown Student",
      studentId: attendanceRecord.studentId || studentInfo?.studentId,
      email: studentInfo?.email || "N/A",
      timeJoined: new Date(
        attendanceRecord.date || attendanceRecord.time || Date.now()
      ).toLocaleString(),
      status: attendanceRecord.status || "Present",
      attendanceType:
        attendanceRecord.method === "QR Code Scan" ? "qr_scan" : "location",
      location: attendanceRecord.location?.coordinates || {
        latitude: 0,
        longitude: 0,
      },
      distance: attendanceRecord.location?.distance
        ? `${attendanceRecord.location.distance}m`
        : "N/A",
    ...
    setSnackbarMessage(
      `⚡ Pulse detected: ${newRecord.studentName} is present`
    );
  ...
  const uniqueMeetings = [
    ...new Set(attendanceData.map((item) => item.meetingId)),
  ];
  ...
  // Load attendance data from backend API using axios
  const loadAttendance = async () => {
    try {
      setLoading(true);
      // Format date for API call
      const dateStr = date.toISOString().split("T")[0];
      // Call backend API to get all attendance logs
      ...
      if (data.success && data.logs) {
        let transformedData = data.logs.map((record, index) => ({
        ...
        if (selectedMeeting !== "all") {
          transformedData = transformedData.filter(
            (item) => item.meetingId === selectedMeeting
          );
        }
      ...
      <Box sx={{ p: 3 }}>
        ...
        <Box sx={{ mb: 4 }}>
          ...
          <Typography variant="body1" color="text.secondary">
            Track the heartbeat of student attendance with instant QR scan
            detection and live updates
          </Typography>
          {connectionStatus !== "connected" && (
            <Alert severity="warning" sx={{ mt: 1 }}>
              ⚡ Pulse signal interrupted - Live monitoring temporarily
              unavailable
            </Alert>
          )}
        </Box>
                  ...
                  attendanceData.map((entry) => (
                    ...
                    >
                      ...
                      <TableCell>
                        <Stack spacing={0.5}>
                          <Tooltip
                            title={`Generated by ${entry.qrGeneratedBy}`}
                          >
                            <Chip
                              icon={<QrCodeIcon />}
                              label="QR Scan"
                          ...
                          <Tooltip
                            title={`Distance: ${entry.distance} from center`}
                          >
                            <Typography
                              variant="caption"
                              color="text.secondary"
